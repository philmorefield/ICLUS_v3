...
CENSUS_FOLDER = 'D:\\OneDrive\\Dissertation\\data\\Census\\center_of_population\\2020'
...
def joint_attrs_to_near_table(near_table):
    ...
    population_shps_path = os.path.join(GDB, 'CenPop2020')
    ...
    df.to_sql(name='county_to_county_distance_2020',
              con=con,
              if_exists='replace',
              index=False)
    con.close()
...
def main():
    csv = os.path.join(CENSUS_FOLDER, 'CenPop2020_Mean_CO.txt')
    df = pd.read_csv(csv)
    df['COFIPS'] = df['STATEFP'].astype('str').str.zfill(2) + df['COUNTYFP'].astype('str').str.zfill(3)
    df.drop(labels=['STATEFP', 'COUNTYFP'], axis=1, inplace=True)
    df = df[df.COFIPS.astype('int') < 57000]
    ...
    df = df[['COFIPS', 'POPULATION', 'LATITUDE', 'LONGITUDE']]
    p1 = df.POPULATION.sum().sum()
    df['SUM_POP'] = df['POPULATION'].groupby(df['COFIPS']).transform('sum')
    df['LAT_x_POP'] = df['LATITUDE'] * df['POPULATION']
    df['LON_x_POP'] = df['LONGITUDE'] * df['POPULATION']
    ...
    fc = os.path.join(GDB, 'CenPop2020')
    ...
    out_table = os.path.join(GDB, 'CenPop_2020_CY_API_Near_Table')
    ap.GenerateNearTable_analysis(in_features=fc,
                                  ...
                                  closest_count=0,
                                  method="GEODESIC")
    joint_attrs_to_near_table(near_table=out_table)
