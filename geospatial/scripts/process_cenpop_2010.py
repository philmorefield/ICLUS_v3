...
CENSUS_FOLDER = 'D:\\OneDrive\\Dissertation\\data\\Census\\center_of_population\\2010'
...
def join_attrs_to_near_table(near_table):
    ...
    population_shps_path = os.path.join(GDB, 'CenPop2010')
    ...
    df.to_sql(name='county_to_county_distance_2010',
              con=con,
              if_exists='replace',
              index=False)
    con.close()
...
def main():
    csv = os.path.join(CENSUS_FOLDER, 'CenPop2010_Mean_CO.txt')
    df = pd.read_csv(filepath_or_buffer=csv, encoding='latin-1')
    ...
    fc = os.path.join(GDB, 'CenPop2010')
    ...
    out_table = os.path.join(GDB, 'CenPop_2010_CY_API_Near_Table')
