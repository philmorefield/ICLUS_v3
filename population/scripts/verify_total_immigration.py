...
def get_interpolated_net_immigration_wittgenstein_v3():
    csv = os.path.join(BASE_FOLDER, 'inputs\\raw_files\\Wittgenstein\\v3\\wcde_asmig.csv')
    ...
    df = df.loc[df.SCENARIO == SCENARIO, ['YEAR', 'NETMIG_INTERP']]
    df = df.rename(columns={'NETMIG_INTERP': 'IMMIGRANTS'})
    df = df.groupby(by='YEAR', as_index=False).sum()

    return df

def get_net_migration_iclus_v3():
    # future immigration
    ...
    df = df.drop(columns=['GEOID', 'RACE', 'SEX', 'AGE_GROUP']).sum().T.reset_index()
    df.columns = ['YEAR', 'IMMIGRANTS']
    df['YEAR'] = df['YEAR'].astype(int)
...
def main():
    wittv3 = get_interpolated_net_immigration_wittgenstein_v3()
    iclusv3 = get_net_migration_iclus_v3()

    wittv3.plot(x='YEAR', y='IMMIGRANTS')
    iclusv3.plot(x='YEAR', y='IMMIGRANTS', ax=plt.gca())
