...
def set_launch_population(launch_year):
    ...
    con = sqlite3.connect(database=POP_DB, timeout=30)
...
class Projector():
    ...
    def __init__(self, scenario):
        ...
        self.net_migration_r = None  # remainder after rounding gross migration flows

        # fertility-related attributes
        self.births = None
        self.births_r = None

        # aging-related attributes
        self.aging_r = None

    def run(self, launch_year=2015, final_projection_year=2017):
        '''
        TODO: all of the various remainder dataframets ('_r') should be addeded
        together at the end of each time step to see if they equal an entire
        person.
        '''
        ...
        while self.current_projection_year <= final_projection_year:
            ...
            # calculate deaths
            self.mortality()  # creates self.deaths and self.deaths_r
            ...
            self.immigration()  # creates self.immigrants and self.immigrants_r
            ...
            if self.current_pop_r is None:
                if self.current_pop.query('VALUE < 0').shape[0] > 0:
                    self.current_pop_r = self.current_pop.query('VALUE < 0').copy()
            else:
                self.current_pop = self.current_pop.add(other=self.current_pop_r, axis='index', fill_value=0)
                self.current_pop_r = self.current_pop.query('VALUE < 0').copy()
            ...
            self.migration()  # creates self.net_migration and self.net_migration_r
            ...
            # calculate births
            self.fertility()  # create self.births and self.births_r
            ...
            db = os.path.join(OUTPUT_FOLDER, 'wittgenstein_v2.sqlite')
            ...
            self.deaths_r.to_sql(name=f'deaths_r_{self.scenario}',
                                 con=con,
                                 if_exists='replace')

            self.immigrants_r.to_sql(name=f'immigrants_r_{self.scenario}',
                                     con=con,
                                     if_exists='replace')

            self.net_migration_r.to_sql(name=f'net_migration_r_{self.scenario}',
                                        con=con,
                                        if_exists='replace')

            self.births_r.to_sql(name=f'births_r_{self.scenario}',
                                 con=con,
                                 if_exists='replace')

            self.current_pop_r.to_sql(name=f'current_pop_r_{self.scenario}',
                                      con=con,
                                      if_exists='replace')

            self.aging_r.to_sql(name=f'aging_r_{self.scenario}',
                                con=con,
                                if_exists='replace')
            con.close()
    ...
    def advance_age_groups(self):
        ...
        if self.aging_r is not None:
            self.current_pop = self.current_pop.add(self.aging_r, axis='index')
        self.aging_r = self.current_pop - self.current_pop.round()
    ...
    def mortality(self):
        ...
        con = sqlite3.connect(database=CDC_DB, timeout=30)
        ...
        con = sqlite3.connect(database=WITT_DB, timeout=30)
        ...
        # store deaths remainders
        if self.deaths_r is None:
            self.deaths_r = df.sub(other=df.round().astype(int), axis='index')
        else:
            df = df.add(other=self.deaths_r, axis='index')
            self.deaths_r = df.sub(other=df.round().astype(int), axis='index')
        assert not df.isnull().any().any()
    ...
    def immigration(self):
        ...
        con = sqlite3.connect(database=ACS_DB, timeout=30)
        ...
        if self.scenario == 'SSP2':
            ...
            raise Exception
        con = sqlite3.connect(database=CENSUS_DB, timeout=30)
        query = f'SELECT * \
        ...
        # store immigration remainders
        if self.immigrants_r is None:
            self.immigrants_r = df.sub(other=df.round().astype(int), axis='index')
        else:
            # add immigration remainders from previous year(s) and recalculate
            # self.immigration_r
            df = df.add(other=self.immigrants_r, axis='index')
            assert not df.isnull().any().any()
            self.immigrants_r = df.sub(other=df.round().astype(int), axis='index')
    ...
    def migration(self):
        ...
        if self.net_migration_r is not None:
            self.net_migration = self.net_migration.add(other=self.net_migration_r, axis='index')
            assert not self.net_migration.isnull().any().any()
        self.net_migration_r = self.net_migration.sub(other=self.net_migration.round().astype(int), axis='index')
        assert self.net_migration_r.shape == (671760, 1)
    ...
    def fertility(self):
        ...
        # store births remainders
        if self.births_r is None:
            self.births_r = births.sub(other=births.round().astype(int), axis='index')
        else:
            births = births.add(other=self.births_r, axis='index')
            self.births_r = births.sub(other=births.round().astype(int), axis='index')
        assert not df.isnull().any().any()
