...
20251110 - p1v01: Use CBO 2025 population estimate as the launch population
"""
import os
import time
...
OUTPUT_DATABASE = os.path.join(OUTPUT_FOLDER, 'p1v01.sqlite')
...
def bin_age_groups(df):
    '''
    Bin single year ages into 5-year age groups
    '''
    df = df.with_columns(pl.when(pl.col('AGE') <= 4)
                         .then(pl.lit('0-4'))
                         .when(pl.col('AGE').is_between(5, 9))
                         .then(pl.lit('5-9'))
                         .when(pl.col('AGE').is_between(10, 14))
                         .then(pl.lit('10-14'))
                         .when(pl.col('AGE').is_between(15, 19))
                         .then(pl.lit('15-19'))
                         .when(pl.col('AGE').is_between(20, 24))
                         .then(pl.lit('20-24'))
                         .when(pl.col('AGE').is_between(25, 29))
                         .then(pl.lit('25-29'))
                         .when(pl.col('AGE').is_between(30, 34))
                         .then(pl.lit('30-34'))
                         .when(pl.col('AGE').is_between(35, 39))
                         .then(pl.lit('35-39'))
                         .when(pl.col('AGE').is_between(40, 44))
                         .then(pl.lit('40-44'))
                         .when(pl.col('AGE').is_between(45, 49))
                         .then(pl.lit('45-49'))
                         .when(pl.col('AGE').is_between(50, 54))
                         .then(pl.lit('50-54'))
                         .when(pl.col('AGE').is_between(55, 59))
                         .then(pl.lit('55-59'))
                         .when(pl.col('AGE').is_between(60, 64))
                         .then(pl.lit('60-64'))
                         .when(pl.col('AGE').is_between(65, 69))
                         .then(pl.lit('65-69'))
                         ...
                         .then(pl.lit('80-84'))
                         .otherwise(pl.lit('85+'))
                         .alias('AGE_GROUP').cast(pl.Enum(AGE_GROUPS)))
