...
def interpolate_years(df):
    ...
    for scenario, age_group, gender in product(scenarios, age_groups, genders):
        ...
        df.loc[(df.SCENARIO == scenario) & (df.AGE_GROUP == age_group) & (df.GENDER == gender), 'MORT_INTERP'] = s.interpolate(method='linear', limit_direction='both')
    ...
    df.loc[(df.GENDER == 'FEMALE') & (df.AGE_GROUP == '5-9') & (df.MORT_CHANGE_MULT.isnull())] = 1.0
...
def combine_85plus(df):
    ...
    older.loc[:, 'NUMERATOR'] = older.groupby(by=['SCENARIO', 'YEAR', 'GENDER'])['MORT_x_WEIGHT'].transform('sum')
    older.loc[:, 'DENOMENATOR'] = older.groupby(by=['SCENARIO', 'YEAR', 'GENDER'])['WEIGHT'].transform('sum')
...
def main():
    ...
    df = df[['YEAR', 'SCENARIO', 'AGE_GROUP', 'GENDER', 'SURV_RATIO', 'MORT_RATE_K', 'MORT_INTERP', 'MORT_CHANGE_MULT']]
