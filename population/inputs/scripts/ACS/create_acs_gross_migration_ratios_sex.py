...
def make_fips_changes(df):
    ...
    #                  as_index=False)
    #         .agg(O_POP_min=pd.NamedAgg(column='O_POP', aggfunc='min'),
    #              O_POP_max=pd.NamedAgg(column='O_POP', aggfunc='max'),
    #              FLOW_sum=pd.NamedAgg(column='FLOW', aggfunc='sum')))

    df = (df.groupby(by=['ORIGIN_FIPS', 'DESTINATION_FIPS', 'SEX'],
                as_index=False)
            .agg(ORIGIN_POPULATION=pd.NamedAgg(column='O_POP', aggfunc='min'),
...
def get_acs_2011_2015_migration():
    ...
    df = df.loc[~df.O_STFIPS.isin(foreign), ['D_STFIPS', 'D_COFIPS', 'O_STFIPS', 'O_COFIPS', 'SEX', 'O_POP', 'FLOW']]
    ...
    df = df[['ORIGIN_FIPS', 'DESTINATION_FIPS', 'SEX', 'O_POP', 'FLOW']]
