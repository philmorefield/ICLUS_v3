...
POPULATION_DB = os.path.join(INPUT_FOLDER, 'databases', 'population.sqlite')
...
def make_fips_changes(df):
    ...
    df = df.groupby(by=['GEOID', 'AGE_GROUP', 'RACE', 'SEX'], as_index=False).sum()

    return df


def main():
    csv = os.path.join(INPUT_FOLDER, 'raw_files\\Census\\2023\\cc-est2023-alldata.csv')
    ...
    df = df.melt(id_vars=['GEOID', 'AGE_GROUP'], var_name='RACE_SEX', value_name='POPULATION')
    df['RACE'] = df['RACE_SEX'].apply(lambda x: x.split('_')[0])
    df['SEX'] = df['RACE_SEX'].apply(lambda x: x.split('_')[1])
    df.drop(columns='RACE_SEX', inplace=True)
    df.RACE = df.RACE.map(RACE_MAP)
    df = df[['GEOID', 'AGE_GROUP', 'RACE', 'SEX', 'POPULATION']]
    df['POPULATION'] = df['POPULATION'].astype(int)

    df = make_fips_changes(df)

    con = sqlite3.connect(database=POPULATION_DB)
    df.to_sql(name='county_population_ageracesex_2020',
              if_exists='replace',
              con=con, index=False)
    con.close()
