...
class Window(QDialog):
    ...
    def get_irs_time_series(self, origin_fips=None, destination_fips=None):
        '''build a time series of IRS migration data'''
        if origin_fips is None or destination_fips is None:
            self.origin_fips = 99999
            self.destination_fips = 99999
            while int(self.origin_fips) > 57000 or int(self.destination_fips) > 57000 or (self.origin_fips == self.destination_fips):
                row = df.sample(n=1)
                self.origin_fips = row.iloc[0]['ORIGIN_FIPS']
                self.destination_fips = row.iloc[0]['DESTINATION_FIPS']
            sub = df.loc[(df['ORIGIN_FIPS'] == self.origin_fips) & (
                df['DESTINATION_FIPS'] == self.destination_fips), :].copy()
        else:
            self.origin_fips = origin_fips
            self.destination_fips = destination_fips
            sub = df.loc[(df['ORIGIN_FIPS'] == self.origin_fips) & (
                df['DESTINATION_FIPS'] == self.destination_fips), :].copy()
            row = sub.sample(n=1)

        sub['EXEMPTIONSY1'].fillna(0, inplace=True)
        sub['EXEMPTIONSY2'].fillna(0, inplace=True)
        sub['EXEMPTIONS'].fillna(0, inplace=True)
        sub['EXEMPTIONS'] = sub[['EXEMPTIONSY1',
                                 'EXEMPTIONSY2', 'EXEMPTIONS']].max(axis=1)
